import 'package:flutter/cupertino.dart'; //for iconsData
...
void populateWordsList(List<String> words, int quantity) {
  if (words.isNotEmpty) words.clear();
  generateNoun().take(quantity).forEach((word) {
    words.add(word.word);
  });
}

void generateNumbers(List<int> _numbers, {int qtd = 1000}) {
  if (_numbers.isNotEmpty) _numbers.clear();
  for (int i = 1; i < qtd; i++) _numbers.add(i);
}

void getIconsNamesFromMap(List<String> randomKeys, int qtd) {
  if (randomKeys.isNotEmpty) randomKeys.clear();
  for (int i = 0; i < qtd; i++)
    randomKeys.add(IconsMap.keys.elementAt(randomNumber(0, IconsMap.length)));
}

IconData getIcon(String key) {
  if (!IconsMap.containsKey(key)) return Icons.ac_unit;
  return IconsMap[key];
}
